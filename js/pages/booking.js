...
function updatePriceSummary() {
    ...
    if (tourInfo) {
        const travelersInfoP = Array.from(tourInfo.querySelectorAll('p')).find(p => 
            p.textContent.includes('Adult')
        );
        if (travelersInfoP) {
            const textNode = travelersInfoP.childNodes[travelersInfoP.childNodes.length - 1];
            if (textNode) {
                textNode.textContent = `${travelerCount} ${travelerCount === 1 ? 'Adult' : 'Adults'}`;
            }
        }
    }
}
...
function handleFormSubmit(e) {
    ...
    alert('Booking submitted successfully! Check console for booking data.');
